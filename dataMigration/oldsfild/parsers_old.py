...
def parse_products_fast(rows):
    """
    Single-threaded version of parse_products().
    Avoids ThreadPoolExecutor overhead inside a background thread.
    """
    products = []
    for r in rows:
        try:
            ...
            # Detect shifted rows (same logic as original parse_products)
            product_name = ""
            packing = ""
            company = ""
...
def parse_product_master_text(text_content):
    ...
    print(f"Total lines in text content: {len(lines)}")
