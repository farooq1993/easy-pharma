...
class OpeningStock(TenantAwareModel):
    ...
    def generate_voucher_number(cls, tenant, opening_stock_date=None):
        """
        Sequential voucher number for Opening Stock.
        Format: OS-YYYY-XXXXXX  e.g. OS-2026-000042
        """
        ...
        last = (
            ...
            .filter(
                tenant=tenant,
                opening_stock_date__range=(fy_start, fy_end),
                voucher_number__startswith=f"OS-{fy_year}-"
            )
            .order_by('-voucher_number')
            .first()
        )
        ...
        return f"OS-{fy_year}-{str(last_seq + 1).zfill(6)}"

    def __str__(self):
        return f"Opening Stock {self.voucher_number or ''}"

class OpeningStockItem(TenantAwareModel):
    opening_stock = models.ForeignKey(OpeningStock, on_delete=models.CASCADE, related_name='items')
    product = models.ForeignKey(Products, on_delete=models.CASCADE)
    batch_number = models.CharField(max_length=100)
    expiry_date = models.DateField()
